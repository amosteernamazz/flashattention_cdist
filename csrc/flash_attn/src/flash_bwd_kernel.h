...
inline __device__ void compute_dq_dk_dv_1colblock(const Params &params, const int bidb, const int bidh, const int n_block) {
    ...
    index_t row_bh_off_q2; index_t row_bh_off_k2; // offset related to batch and nheads

    Tensor gQ = make_tensor(make_gmem_ptr(reinterpret_cast<Element *>(params.q_ptr) + row_offset_q),
                            Shape<Int<kBlockM>, Int<kHeadDim>>{},
                            make_stride(params.q_row_stride, _1{}));

    Tensor gQaccum = make_tensor(make_gmem_ptr(reinterpret_cast<ElementAccum *>(params.q_ptr) + row_offset_q),
                                  Shape<Int<kBlockM>, Int<kHeadDim>>{},
                                  make_stride(params.q_row_stride, _1{}));
    ...
    if(cute::thread0()){print("sdS layout: ");print(sdS.layout());print("\n");}
    ...
    Tensor tdQgdQ = gmem_thr_copy_dQ.partition_D(gdQ);
    Tensor tdQgdQaccum = gmem_thr_copy_dQ_accum.partition_D(gdQaccum);
    Tensor tQ_tmp = gmem_thr_copy_dQ_accum.partition_D(gQ);
    Tensor tQgQaccum = gmem_thr_copy_dQ_accum.partition_D(gQaccum);
    ...
    for (; m_block >= m_block_min; --m_block) {
        ...
        if (params.use_cdist){
            row_bh_off_q2 = blockIdx.y * (params.seqlen_q_rounded*params.h) + 
                        blockIdx.z * (params.seqlen_q_rounded); 
            row_bh_off_k2 = blockIdx.y * (params.seqlen_k_rounded*params.h_k) + 
                        blockIdx.z * (params.seqlen_k_rounded);

            flash::apply_cdist(scores, 
                row_bh_off_k2 + n_block * kBlockN + (tidx / 32 / AtomLayoutMS) * MMA_N_SdP * 16, binfo.actual_seqlen_k,
                row_bh_off_q2 + m_block * kBlockM + get<0>(taccScS_row(0)),
                AtomLayoutMS * 16,
                reinterpret_cast<Element *>(params.q2_ptr),
                reinterpret_cast<Element *>(params.k2_ptr));
        }
        ...
        if(cute::thread0()){print("dS layout: "); print(dS.layout()); print("\n");}
        // if(cute::thread0()){print(dS);}
        auto pointwise_mult = [](float p, float dp, float d) {
            return p * (!Is_dropout || p >= 0 ? dp - d : d);
        };

        auto pointwise_dev = [](float dS, float s){
            return (dS / s);
        };
        // if(cute::thread0()){print("dS size: ");print(dS.size());}

        // if(cute::thread0()){print("scores: "); print("\n"); print(scores);print("\n"); print("ds: "); print("\n"); print(dS); print("\n"); print("dp_sum: "); print("\n"); print(dP_sum);}
        
        // if(cute::thread0()){print("before pointwise_mult: "); print("\n");print("dS: "); print("\n"); print(dS);print("\n"); print("S: "); print("\n"); print(scores);}
        #pragma unroll
        ...
        // if(cute::thread0()){print("before devided: "); print("\n");print("dS: "); print("\n"); print(dS);print("\n"); print("S: "); print("\n"); print(scores);}
        ...
        for(int i = 0; i < size(dS); ++i){
                dS(i) = pointwise_dev(dS(i), scores(i));
            }
        }
    ...
    for (; m_block >= m_block_min; --m_block) {
        ...
        // if(cute::thread0()){print("after devided: "); print("\n");print("dS: "); print("\n"); print(dS);print("\n");  print("S: "); print("\n"); print(scores);}
        ...
        // if (cute::thread0()) { print("tdQrdS layout");print(tdQrdS.layout());print("\n"); }
        // if (cute::thread0()) { print("dS layout");print(dS.layout());print("\n"); }
        // if (cute::thread0()) { print("tdQsdS layout");print(tdQsdS.layout());print("\n"); }
        // if (cute::thread0()) { print("tdQgdQaccum layout");print(tdQgdQaccum.layout());print("\n"); }
        // if (cute::thread0()) { print("acc_dq layout");print(acc_dq.layout());print("\n"); }        
        // if (cute::thread0()) { print("tdQrKt layout");print(tdQrKt.layout());print("\n"); }
        // if (cute::thread0()) { print("size<0>(tdQrdS) layout");print(size<0>(tdQrdS));print("\n"); }
        // if (cute::thread0()) { print("size<0>(tdQrdS) layout");print(size<1>(tdQrdS));print("\n"); }
        // if (cute::thread0()) { print("size<0>(tdQrdS) layout");print(size<2>(tdQrdS));print("\n"); }
        // flash::gemm(acc_dq, tdQrdS, tdQrKt, tdQsdS, tdQsKt, tiled_mma_dq, smem_thr_copy_dS, smem_thr_copy_Kt);
        // if (cute::thread0()) { print("tSrQ layout ");print(tSrQ.layout()); print("\n"); }
        if (params.use_cdist){
            // flash::apply_score_rowsum(dS, 
            //     row_bh_off_k2 + n_block * kBlockN + (tidx / 32 / AtomLayoutMS) * MMA_N_SdP * 16, binfo.actual_seqlen_k,
            //     row_bh_off_q2 + m_block * kBlockM + get<0>(taccScS_row(0)),
            //     AtomLayoutMS * 16,
            //     reinterpret_cast<Element *>(params.s_rum_ptr));


            // if (cute::thread0()) {print("tSrQ: ");print(tSrQ); print("\n");}

            // if (cute::thread0()){print("-------------------");print("\n");}
            // if (cute::thread0()) {print("tdQrdS: ");print(tdQrdS); print("\n");}
            // if (cute::thread0()) {print("tdQrKt: ");print(tdQrKt); print("\n");}
            // if (cute::thread0()) {print("acc_dq: ");print(acc_dq); print("\n");}
            // if (cute::thread0()) {print("tdKrQt: ");print(tdKrQt); print("\n");}

            // if (cute::thread0()) {print("tSrQ: ");print(tSrQ); print("\n");}

            // 存在问题1： 如何将dS对应到tdQgdQaccum
            #pragma unroll 
            for(int mi = 0; mi < size<0>(dS); ++mi){
                float scalar = 0;
                #pragma unroll
                ...
                #pragma unroll
                for(int ni = 0 ; ni < size<2>(tdQgdQaccum); ++ni){
                tdQgdQaccum(0, mi, ni) = tdQgdQaccum(0, mi, ni) + Element(scalar * /*修改：Qi*/tQgQaccum(0, mi, ni));
                }
            }
            // #pragma unroll
            // for(int mi = 0; mi < size<0>(tdQrdS); ++mi){
            //     #pragma unroll
            //     float scalar = 0;
            //     for(int ni = 0 ; ni < size<1>(tdQrdS); ++ni){
            //         for(int pi = 0; pi < size<2>(tdQrdS); ++pi){
            //             scalar += tdQrdS(mi, ni, pi);
            //         }
            //         // scalar += tdQrdS(mi, ni,);
            //     }
            //     #pragma unroll
            ...
            //     }
            // }
            // if (cute::thread0()) {print("tdQgdQaccum: ");print(tdQgdQaccum); print("\n");}
            // 存在问题2：目前当前代码中求解gemm的对象是S并非dS，
            for (int i = 0; i < size(acc_dq); ++i) { atomicAdd(&tdQgdQaccum(i), -2*acc_dq(i)); } // 
        }
        //tdQgdQaccum: tile dq from gdQ, accumulator
        ...
        if (params.use_cdist){
            ...
            for(int ni = 0 ; ni < size<1>(dS); ++ni){
                ...
                #pragma unroll
                // element wise： 目前找不到如何对k的取值

            }
        }

        if (cute::thread0()) { print("acc_dk: ");print(acc_dk); }
        if (cute::thread0()) { print("tdKrdSt: ");print(tdKrdSt); }
