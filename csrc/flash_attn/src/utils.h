...
////////////////////////////////////////////////////////////////////////////////////////////////////
// apply cdist instead of q*k.T
template <typename Engine, typename Layout, typename Element>
inline __device__ void apply_cdist(Tensor<Engine, Layout> &tensor, const uint32_t col_idx_offset_,
                                         const uint32_t max_seqlen_k, const uint32_t row_idx_offset_,
                                        const uint32_t warp_row_stride,
                                        Element q2_ptr, Element k2_ptr) {
        ...
        for (int i = 0; i < size<0, 0>(tensor); ++i) {
            ...
            for (int nj = 0; nj < size<1, 1>(tensor); ++nj) {
                ...
                for (int j = 0; j < size<1, 0>(tensor); ++j) {
                    ...
                    -sqrt(q2_ptr[row_idx] + k2_ptr[col_idx] -2*tensor(make_coord(i, mi), make_coord(j, nj)));
                }
            }
        }
    }
