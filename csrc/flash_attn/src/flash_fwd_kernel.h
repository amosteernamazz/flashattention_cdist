...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    // if (cute::thread0()) {printf("params.use_cdist (print from kernel) %d\n", params.use_cdist);}
    // Shared memory.
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (params.use_cdist){
            ...
            flash::apply_cdist(scores, 
                row_bh_off_k2 + n_block * kBlockN, binfo.actual_seqlen_k,
                row_bh_off_q2 + m_block * kBlockM + (tidx / 32) * 16 + (tidx % 32) / 4,
                kNWarps * 16,
