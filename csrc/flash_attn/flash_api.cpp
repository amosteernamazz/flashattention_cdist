...
mha_fwd(const at::Tensor &q,         // batch_size x seqlen_q x num_heads x head_size
        ...
        c10::optional<bool> cdist_,
        c10::optional<at::Tensor> &q2_,
        c10::optional<at::Tensor> &k2_) {
    ...
    if (cdist_.has_value()){
        params.use_cdist = cdist_.value();
        if (cdist_.value() == true){
            if (q2_.has_value() && k2_.has_value()){
                q2 = q2_.value(); k2 = k2_.value();
                TORCH_CHECK(q2.dtype() == q_dtype, "q^2 must have the same dtype as inputs");
                TORCH_CHECK(q2.is_cuda(), "q^2 tensor must be on CUDA device");
                // TORCH_CHECK(q2.stride(-2) == q.stride(-2), "q^2 tensor must have the same last dimension as q do");
                // CHECK_SHAPE(q2, batch_size, seqlen_q, num_heads, head_size_og);

                TORCH_CHECK(k2.dtype() == q_dtype, "k^2 must have the same dtype as inputs");
                TORCH_CHECK(k2.is_cuda(), "k^2 tensor must be on CUDA device");
                // TORCH_CHECK(k2.stride(-2) == k.stride(-2), "k^2 tensor must have the same last dimension as k do");
                // CHECK_SHAPE(k2, batch_size, seqlen_q, num_heads, head_size_og);

                params.q2_ptr = q2.data_ptr();
                params.k2_ptr = k2.data_ptr();
                // printf("Using cidst, only support euclidien dist\n");
            }
            else{
                TORCH_CHECK(0, "q^2 k^2 must be passes");
            }
...
mha_bwd(const at::Tensor &dout,  // batch_size x seqlen_q x num_heads, x head_size_og
    ...
    if(cdist_.has_value()){
        ...
        if (cdist_.value() == true){
            ...
            // if (s_sum_.has_value() ){
            //     s_sum = s_sum_.value();
            //     params.s_sum_ptr = s_sum.data_ptr();
            // }
        }
